...
**Phases 1, 2, and 3 are complete and accepted** (steps 00–20). Phase 1 (00–09): the shared domain, the
...
WebRTC; `DraftStarted` is appended as the durable fact), the send flow (the body is
snapshotted into an immutable `MessageSent`; every append is advertised to all peers),
read-only client event consumption (offset-paged catch-up, including after a
reconnect; the conversation timeline renders from the event projection alone), and the
agent turn (a streamed response represented entirely as events; the Claude Agent SDK
adapter is exercised by a key-gated live smoke test while the deterministic suite
injects scripted runners). The model/protocol suites and the event-driven WebRTC E2Es
are green: the Step 09 gate certifies all seven required E2E scenarios, the protocol
invariants, and the UI checklist from the single `mise run test` entry point,
repeatably.
...
Phase 1 regression, from the same entry point.

Phase 3 (15–20) adds turn discipline and full local-first durability: sending enqueues
into a shared, CRDT-backed **message queue** — queued messages stay editable,
reorderable (fractional index), and deletable by any peer until the Session Process
**drains** them into immutable `MessageSent` events (the linearization point; while
the agent works, new sends queue Cursor-style, and one coalesced turn runs per batch);
an explicit **interrupt** cancels the running turn (partial response kept) and drains
immediately. The concurrency contract — exactly-once consumption, snapshot fidelity,
terminal immutability, convergence, total order, liveness, single-flight — is verified
by Hedgehog property schedules against a shadow-replica oracle, plus named race tests
for delete/edit/reorder-vs-accept in both orderings. The Yjs document is durable on
both sides: a sidecar update log in the Session Process (replayed and compacted at
boot; the crash window between append and removal is repaired via log-anchored dedup)
and IndexedDB in the browser (a draft survives a full server wipe, proven in the
browser E2E). See the [tracker](docs/plans/TODO.md) for details and the decisions log.
