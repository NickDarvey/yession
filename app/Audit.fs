module Yession.Host.Audit

// The Manager's audit telemetry (Plan 06): OTel-log-shaped records the Manager emits IN-PROCESS
// about its own authority decisions — secrets operations, policy denies, SecretRef injection,
// KEK/store lifecycle, user↔launch bindings, and control-channel 401s. Identifiers and names
// only — no constructor accepts a secret value, KEK material, or control secret, so content
// cannot leak by type. Records carry `event.name` (`yession.*`) and `service.name = yession-manager`.
//
// (Session telemetry does not flow through the Manager — every process is a direct OTel emitter,
// app/Telemetry.fs — so there is no receiver or collector here; the Manager just emits its own
// audit records, one greppable stdout line each via `stdout`.)
