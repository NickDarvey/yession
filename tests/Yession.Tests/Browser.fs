...
let editorTests =
    testList "Editor rendering (browser)" [
        ...
        // Hold a finger on a message and its actions open. Only a browser can answer this at
        // all: it is a listener, a timer and three ways to be cancelled, and none of them is
        // visible in a rendered string.
        //
        // The pointer sequence is DISPATCHED rather than driven through the input pipeline,
        // and that is a real limit worth naming: Playwright's touchscreen offers a tap, not a
        // hold, so a held finger has to be assembled from events. What runs is the product's
        // own listener over real `PointerEvent`s; what is not covered is the platform's own
        // behaviour around one — a selection starting, or the OS menu — which no headless
        // browser reproduces and only a device can settle.
        editorCaseIn 390 844 "holding a finger on a message opens its actions" (EDITOR_PORT + 26) <| fun page ->
            async {
        ...
        // The half that keeps the gesture from being "any touch at all". A tap is how every
        // control on this surface is used, so a tap that opened a menu would put one over
        // every press a person makes.
        editorCaseIn 390 844 "a tap on a message opens nothing" (EDITOR_PORT + 27) <| fun page ->
            async {
                let! _ = await (page.WaitForSelectorAsync "#shell [data-conversation] [data-message-id]")
                let! _ =
                    await (page.EvaluateAsync<bool>
                ...
                // Longer than the hold, so this is "it never fired" and not "it had not fired
                // yet" — the one shape a wait-for-absent can otherwise pass by being early.
                do! Async.Sleep 900
                let! menus = await (page.EvaluateAsync<int> "() => document.querySelectorAll('#shell [data-item-menu]').length")
                Expect.equal menus 0 "a finger down and up is a tap, and a tap is not a hold"
            }
        // And the way a hold most often ends up not being one: the list starts moving. The
        // finger has not moved relative to the screen, so nothing but the scroll itself says
        // the gesture is over.
        editorCaseIn 390 844 "a hold that turns into a scroll opens nothing" (EDITOR_PORT + 28) <| fun page ->
            async {
                let! _ = await (page.WaitForSelectorAsync "#shell [data-conversation] [data-message-id]")
                let! _ =
                    await (page.EvaluateAsync<bool>
                            """() => {
                                 ...
                                 const at = { clientX: b.left + 20, clientY: b.top + 8, pointerType: 'touch', bubbles: true }
                                 item.dispatchEvent(new PointerEvent('pointerdown', at))
                                 const scroller = document.querySelector('#shell [data-conversation]')
