    ...
    testList "Process doc persistence (Step 19)" [
        testCaseAsync "a restart replays the persisted doc: pending entries drain exactly once at boot; drafts survive and are not re-announced" <|
            async {
                ...
                let! h1 = Host.startFull (Some runner1) None (Some (openLog ())) (Some (DocStore.openStore docPath)) None sessionId "t" 0
                ...
                let! h2 = Host.startFull (Some runner2) None (Some (openLog ())) (Some (DocStore.openStore docPath)) None sessionId "t" 0
                let! secondLife = sentMessages h2.Log
                Expect.equal
