    ...
    testList "Process doc persistence (Step 19)" [
        testCaseAsync "a restart replays the persisted doc: pending entries drain exactly once at boot; drafts survive and are not re-announced" <|
            async {
                ...
                let! h1 = Host.startFull (Some runner1) None None (Some (openLog ())) (Some (DocStore.openStore docPath)) None (fun _ _ -> ()) None None sessionId None 0
                ...
                let! h2 = Host.startFull (Some runner2) None None (Some (openLog ())) (Some (DocStore.openStore docPath)) None (fun _ _ -> ()) None None sessionId None 0
                let! secondLife = sentMessages h2.Log
                Expect.equal
        ...
        testCaseAsync "a torn final line in the doc store is dropped; the acknowledged state is intact" <|
            async {
                ...
                let! h1 = Host.startFull None None None (Some (openLog ())) (Some (DocStore.openStore docPath)) None (fun _ _ -> ()) None None sessionId None 0
                ...
                let! h2 = Host.startFull None None None (Some (openLog ())) (Some (DocStore.openStore docPath)) None (fun _ _ -> ()) None None sessionId None 0
                let synced = SyncedStateSync.ofDoc h2.Doc |> Result.mapError (sprintf "%A") |> expect
                Expect.equal
                    (synced.Drafts |> Map.tryFind oPeer |> Option.map (fun _ -> SyncedStateSync.draftBodyMarkdown h2.Doc oPeer))
                    (Some "acknowledged")
