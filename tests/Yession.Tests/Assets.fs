...
let private withAssets (name: string) (body: Assets.AssetSet -> 'a) : 'a =
    ...
    writeFile nodeFs (dir + "/" + AssetFile.path AssetFile.``neon-300``) "not really a face, but bytes are bytes"
    ...
    testList "Static assets" [
        testCase "a file is served as what the declaration says it is" <| fun () ->
            ...
            withAssets "types" (fun assets ->
                ...
                let face = serveInto assets (buildOf assets) (AssetFile.path AssetFile.``neon-300``)
        ...
        testCase "changing any file changes every address in the set" <| fun () ->
            ...
            writeFile nodeFs (dir + "/" + AssetFile.path AssetFile.``neon-300``) "different bytes entirely"
