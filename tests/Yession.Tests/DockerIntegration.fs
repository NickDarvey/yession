...
// volume, build spec, secret refs). The whole suite sits under `Tag.needs [Docker]` (Node +
// verify tier), which is the ONLY daemon gate: `check` probes for a daemon and drops the
// capability when there is none, so these report one skip instead of running empty — and the
// cheap tier never reaches them. Under YESSION_REQUIRE_DOCKER (release.yml) the capability is
// never dropped, so a gate promised a daemon fails here rather than skipping to green.
...
let tests =
    Tag.needs "Docker integration" [ Tag.Docker ] (fun () ->
        testList "Docker integration" [
            ...
            testCaseAsync "lifecycle: start, stream stdout, stop, container is gone" (async {
            ...
            testCaseAsync "a non-zero command exit maps to CommandFailed" (async {
            ...
            testCaseAsync "two sessions get separately labelled containers" (async {
            ...
            testCaseAsync "a command past its timeout maps to CommandTimedOut" (async {
                let! _, caps, handle = start alpineSpec
            ...
            testCaseAsync "the session workspace named volume persists across a restart" (async {
            ...
            testCaseAsync "secret refs resolve from the store; missing secrets fail start" (async {
            ...
            testCaseAsync "the Manager's secret store injects into the container env, gated by the ABAC walk (Plan 06)" (async {
