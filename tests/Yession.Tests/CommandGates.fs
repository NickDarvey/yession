    ...
    testList "The command gate" [
        ...
        testCaseAsync "an ungated command runs, and answers with what it said" <|
            async {
                ...
                Expect.equal (Seq.head seen).ApprovedBy None "nobody had to approve it"
                Expect.equal (Seq.head seen).OnBehalfOf (Some ada') "on the turn actor's credential"
        ...
        testCaseAsync "a gated command is visible BEFORE it happens, and does not happen unasked" <|
            async {
                ...
                | [ (_, act) ] ->
                    ...
                    Expect.equal act.OnBehalfOf (Some ada') "with whose credential it runs on"
        ...
        testCaseAsync "an approval releases it, and reaches the command's own dispatch" <|
            async {
                ...
                Expect.equal (Seq.head seen).ApprovedBy (Some (PeerRef ada)) "and the event can name who released it"
                let synced = SyncedStateSync.ofDoc doc |> expect
        ...
        testCaseAsync "an approval survives the process that proposed the act" <|
            async {
                ...
                Expect.equal invocation.OnBehalfOf (Some ada') "and the credential owner off the act too"
                Expect.equal invocation.ApprovedBy (Some (PeerRef ada)) "and who released it"
                let synced = SyncedStateSync.ofDoc doc |> expect
        ...
        testCaseAsync "a queued TERMINAL command is not the gate's business" <|
            async {
                ...
                SyncedStateSync.enqueueTerminalCommand doc (QueueId.create "q-t1" |> expect) terminal ActorRef.Agent 1.0 "git status" false
